...
class RegistrationForm(FlaskForm):
    ...
    first_name = StringField('First Name', [DataRequired(), validators.Length(min=2, max=40), validators.Regexp('^[A-Za-z-]+$', message='First name must only contain alphabetic characters and hyphens. First name cannot start or end with a hyphen')])
    last_name = StringField('Last Name', [DataRequired(), validators.Length(min=2, max=40), validators.Regexp('^[A-Za-z-]+$', message='Last name must only contain alphabetic characters and hyphens. Last name cannot start or end with a hyphen')])
...
class LoginForm(FlaskForm):
    ...
    def validate_password(self, field):
        ...
        user = User.find_user_by_email(self.email.data)
