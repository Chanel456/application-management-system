...
class RegistrationForm(FlaskForm):
    ...
    first_name = StringField('First Name', [DataRequired(), validators.Length(min=2, max=150, message='First name cannot exceed 150 characters'), validators.Regexp('^[A-Za-z-]+$', message='First name must only contain alphabetic characters and hyphens. First name cannot start or end with a hyphen')])
    last_name = StringField('Last Name', [DataRequired(), validators.Length(min=2, max=150, message='Last name cannot exceed 150 characters'), validators.Regexp('^[A-Za-z-]+$', message='Last name must only contain alphabetic characters and hyphens. Last name cannot start or end with a hyphen')])
    password = PasswordField('Password', [DataRequired(), validators.Regexp('^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]{7,}$', message='Password must contain at least 1 uppercase letter, 1 number,  1 special character [!@#$%^&*()_+] and be at least 7 characters long'), validators.Length(min=7, max=150, message='Password cannot exceed 150 characters'), validators.EqualTo('confirm_password', message='Passwords must match')])
    confirm_password = PasswordField('Confirm Password', [DataRequired(), validators.Regexp('^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]{7,}$', message='Password must contain at least 1 uppercase letter, 1 number,  1 special character [!@#$%^&*()_+] and be at least 7 characters long'), validators.Length(min=7, max=150, message='Password cannot exceed 150 characters'), validators.EqualTo('password', message='Passwords must match')])
